"""
TinyLlama-1.1B Triton Python Backend Model

This model runs TinyLlama-1.1B-Chat on the Triton server using the Python backend
with HuggingFace Transformers. Use this as a baseline for comparison with
TensorRT-LLM optimized version.

Model: TinyLlama/TinyLlama-1.1B-Chat-v1.0
- 1.1B parameters (~2GB weights)
- Llama architecture (same as Llama 2)
- Apache 2.0 license (open, no gating)

Model weights are loaded from the weights folder:
    <base>/weights/tinyllama-python/1/

Directory structure:
    <base>/
    ├── models/tinyllama-python/      <- Triton loads from here
    │   ├── config.pbtxt
    │   └── 1/model.py (this file)
    └── weights/tinyllama-python/     <- Model weights
        └── 1/<HF model files>
