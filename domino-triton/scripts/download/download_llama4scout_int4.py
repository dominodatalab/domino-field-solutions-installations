#!/usr/bin/env python3
"""
download_llama4scout_int4.py

Downloads RedHatAI/Llama-4-Scout-17B-16E-Instruct-quantized.w4a16 -- an
INT4 weight / 16-bit activation (W4A16) quantization of Llama 4 Scout in
vLLM's native compressed-tensors format (same family as
download_llama4scout_fp8_dynamic.py's checkpoint), so it's expected to load
without the ModelOpt-style naming issues NVIDIA's FP8 checkpoint hit.

Mirrors download_llama4scout_fp8_dynamic.py's structure/conventions for consistency.

Llama 4 Scout is a Mixture-of-Experts model:
- 17B active parameters per forward pass, 109B total parameters, 16 experts
- The INT4 checkpoint is ~61GB on disk (vs ~109GB for the FP8/FP8-dynamic
  checkpoints), so it should fit on a smaller node -- match
  --tensor-parallel-size to your actual GPU count rather than assuming the
  FP8 variants' 8-GPU sizing is needed here.
- 128K token native context window
- Gated by Meta's upstream Llama 4 license -- requires a HuggingFace token

Usage:
    # With HF_TOKEN env var:
    HF_TOKEN=hf_... python scripts/download/download_llama4scout_int4.py

    # With --token flag:
    python scripts/download/download_llama4scout_int4.py --token hf_...

    # Custom base directory (e.g. an S3 FUSE mount or shared NFS path):
    python scripts/download/download_llama4scout_int4.py --token hf_... --base-dir /mnt/s3/triton-repo

    # Custom tensor parallel size (match it to your actual node's GPU count):
    python scripts/download/download_llama4scout_int4.py --token hf_... --tensor-parallel-size 2

    # Skip local verification (faster for pre-validated weights):
    python scripts/download/download_llama4scout_int4.py --token hf_... --no-verify

Requirements:
    pip install huggingface_hub transformers

Output (relative to --base-dir, default: <project-root>/triton-repo):
    <base-dir>/weights/llama4scout-vllm-int4/   <- downloaded HuggingFace weights
    <base-dir>/models/llama4scout-vllm-int4/    <- Triton model directory
      config.pbtxt
...
TRITON_MODEL_NAME = "llama4scout-vllm-int4"
MODEL_VERSION = "1"
APPROX_SIZE_GB = 61  # confirmed via actual download: 84 files, 61GB total. RedHatAI's
# model card claimed ~28GB ("INT4 reduces disk size ~75% vs BF16"), but that appears to
# undercount the vision tower and MoE-router/attention layers this recipe leaves
# unquantized (see the config.pbtxt "ignore" list) -- still much smaller than the
# ~109GB FP8/FP8-dynamic checkpoints, just not the full 75% reduction advertised.


def parse_args():
    parser = argparse.ArgumentParser(
        description="Download RedHatAI's compressed-tensors INT4 (W4A16) Llama-4-Scout checkpoint and set up Triton vLLM model directory"
    )
    parser.add_argument(
        "--token",
    ...
    parser.add_argument(
        ...
        choices=[1, 2, 4, 8],
        help=(
            "Number of GPUs for tensor parallelism (default: 4 -- the ~61GB "
            "checkpoint plus KV cache/activation overhead should fit "
            "comfortably on a 4x A10G/L4 node; override to match your "
            "actual node's GPU count)"
        ),
    )
    parser.add_argument(
        "--max-model-len",
...
def download_model_weights(weights_dir: Path, token: str, ignore_ssl: bool) -> Path:
    ...
    print(f"Approx size: ~{APPROX_SIZE_GB} GB (INT4 compressed-tensors)")
    print(f"{'='*60}\n")

    if not token:
        print("ERROR: HuggingFace token required (the upstream Llama 4 license is gated).")
    ...
    print(f"(This is a ~{APPROX_SIZE_GB}GB download)\n")
...
def main():
    ...
    print("=" * 60)
    print("Llama-4-Scout INT4 (W4A16, compressed-tensors) Setup for Triton vLLM")
    print(f"Model: {MODEL_ID}")
    print("=" * 60)
