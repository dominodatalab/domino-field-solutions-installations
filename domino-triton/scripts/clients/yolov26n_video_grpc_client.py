#!/usr/bin/env python3
"""
YOLOv26n Video Inference Client (gRPC)
...
Usage:
    python yolov26n_video_grpc_client.py --video input.mp4 --output results.json
    python yolov26n_video_grpc_client.py --video input.mp4 --batch-size 4
    python yolov26n_video_grpc_client.py --video input.mp4 --output-video annotated.mp4
    python yolov26n_video_grpc_client.py --video input.mp4 --async  # Async mode
"""
...
RESULTS_DIR = SCRIPTS_DIR.parent.parent / "results" / "yolov26"

INPUT_SIZE = (640, 640)

# COCO 80 class names (same as YOLOv8 for standard COCO-trained models)
COCO_CLASSES = [
...
def preprocess_frame(frame: np.ndarray):
    """
    Preprocess frame for YOLOv26n with letterbox resize.
...
def postprocess_yolo(output: np.ndarray, conf_thres: float = 0.25):
    """
    Parse YOLOv26n output tensor to detections.
    Output shape: [batch, num_classes+4, num_anchors] -> transpose to [batch, num_anchors, num_classes+4]
    ...
    for batch_idx in range(output.shape[0]):
        ...
        # YOLO output is [num_classes+4, num_anchors], transpose to [num_anchors, num_classes+4]
...
def main():
    parser = argparse.ArgumentParser(description="YOLOv26n Video Inference (gRPC)")
    ...
    parser.add_argument("--output", "-o",
                        default=str(RESULTS_DIR / "yolov26n_grpc.json"),
                        help=f"Output JSON file (default: {RESULTS_DIR}/yolov26n_grpc.json)")
    parser.add_argument("--output-video",
                        default=str(RESULTS_DIR / "annotated_grpc.mp4"),
                        help=f"Output annotated video file (default: {RESULTS_DIR}/annotated_grpc.mp4)")
    parser.add_argument("--async", dest="async_mode", action="store_true",
                        help="Use async mode for concurrent batch processing")
