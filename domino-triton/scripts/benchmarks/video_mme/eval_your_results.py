"""
Vendored, unmodified from https://github.com/thanku-all/parse_answer/blob/main/eval_your_results.py
(the official Video-MME scoring script). Kept byte-for-byte identical to
upstream so it stays diffable -- extract_characters_regex(), CATEGORIES,
SUB_CATEGORIES, and TASK_CATEGORIES are reused directly by
benchmark_video_mme.py's compute_accuracy_metrics() rather than duplicated,
since accuracy correctness depends on matching the official answer-extraction
logic exactly. This file's own CLI (`__main__` below) is for producing the
official printed report; benchmark_video_mme.py needs actual numeric values
(for MLflow), which is why that logic lives separately rather than here.
"""
import os
import argparse
import json
