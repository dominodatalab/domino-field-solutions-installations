...
1. Download a model from an external source and conver to ONNX format. [download_yolo.ipynb](notebook/download_yolo.ipynb) demonstrates how to:
   - Download a YOLOv8 model from Ultralytics Hub
   - Convert the model to ONNX format
   - Validate the ONNX model using ONNX Runtime
2. Model registration in Domino. Use the notebook [model_registration_deploy.ipynb](notebook/model_registration_deploy.ipynb) to:
...
3. Model testing using Triton client libraries from a Domino workspace - Use a sample video file, extract frames, and send them to the Triton Inference Server for inference using gRPC.
   Use the notebook [model_testing.ipynb](notebook/model_testing.ipynb) to:
4. Administration of the Triton Inference Server deployment from a Domino workspace . Use the notebook [admin.ipynb](notebook/admin.ipynb) to perform the following admin tasks:
